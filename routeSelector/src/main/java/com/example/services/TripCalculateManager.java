...
public class TripCalculateManager implements Comparator<TripSchedule>{
    ...
    public int compare(TripSchedule o1, TripSchedule o2) {
        ...
        try {
            ...
            else{if(tempselection2.getDelay(timeNow)<(tempselection1.getDelay(timeNow)+20*60*1000) && tempselection2.getTripLength()<tempselection1.getTripLength()) {finalSelection = tempselection2;}}
            //if the delay of second()  is already less than delay of first() return whatever object in hand otherwise switch the selection to other object.
